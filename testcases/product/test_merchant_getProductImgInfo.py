...
class GetProductImgInfo(unittest.TestCase):
    ...
    def __init__(self,methodName='runTest'):
        # 数据初始化
        super(GetProductImgInfo,self).__init__(methodName)
        self.getProductImgInfo_data=ReadExcel().readExcel(r'../getProductImgInfo_api.xlsx','Sheet1')
        for i in range(len(self.getProductImgInfo_data)):
            if self.getProductImgInfo_data[i]['sql']!='' and '{virtual_goods_id}' in self.getProductImgInfo_data[i]['params']:
                ...
                if '{virtual_goods_id}' in self.getProductImgInfo_data[i]['expect_result']:
                    self.getProductImgInfo_data[i]['expect_result'] = self.getProductImgInfo_data[i]['expect_result'].replace('{virtual_goods_id}', ''.join('%s' %id for id in a[i]))
            else:
                continue
