...
class DisableSale(unittest.TestCase):
    ...
    def test_disableSale1(self):
        ...
        i = 0
        while i<len(result):
            disableSale_data = {'token': token, 'goods_list': result[i]}
    ...
    def test_disableSale4(self):
        ...
        i = 0
        while i<len(result):
            disableSale_data = {'token': token, 'goods_list':result[i],'fbv_disable_way':'disable'}
            r = requests.post(url=self.disableSale_url, json=disableSale_data, headers=self.headers)
            if r.json()['execute_status'] == 'failed':
                i += 1
                continue
    ...
    def test_disableSale5(self):
        ...
        i = 0
        while i<len(result):
            disableSale_data = {'token': token, 'goods_list': result[i],'fbv_disable_way':'clear'}
            r = requests.post(url=self.disableSale_url, json=disableSale_data, headers=self.headers)
            if r.json()['execute_status'] == 'failed':
                i += 1
                continue
